...
- `/` — главная лента с клиентскими фильтрами по периоду, каналу, теме и тексту
...
- `/compare/` — таблица каналов с сортировкой по столбцам
- `/channels/{channel}/` — отдельная страница канала только с его постами
- `/lite/` — компактный режим чтения
- `/before/{id}/`, `/after/{id}/` — пагинация
- `/channels/{channel}/before/{id}/`, `/channels/{channel}/after/{id}/` — пагинация внутри канала
- `/search/{query}/` — поиск по тегам и текстовым совпадениям
- `/rss.xml`, `/rss.json` — RSS-фиды

## Нормализация контента

`src/lib/data.ts` и `src/lib/telegram/index.ts` не просто отдают сырой Telegram HTML, а приводят его к более стабильной структуре:

- разбивают legacy-контент с двойными `br` на нормальные абзацы;
- сохраняют telegram emoji и inline media;
- выравнивают blockquote, списки и типографику под единый reader style;
- раскладывают digest-посты на секции, разделители и emoji-списки;
- подготавливают многоизображенческие посты к карусельному отображению.

Это важно, потому что разные каналы публикуют контент в очень разной HTML-структуре, и без нормализации reader визуально «пляшет» от поста к посту.
...
- **Статический сайт**: фильтры на главной работают только по уже загруженным постам на текущей странице.
- **Поиск статически сгенерирован**: для сложных свободных запросов лучше использовать короткие ключевые слова или теги.
